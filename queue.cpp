...
public:
    ...
    bool isEmpty()
    {
        if (front == -1 && rear == -1)
        {
            return true; // If both front and rear are -1, the queue is empty
        }
        else
        {
    ...
    bool isFull()
    {
        if (rear == 4) // Array indices are 0 to 4 (total of 5 elements)
        {
    ...
    int count()
    {
        return rear - front + 1; // Number of elements in the queue
    }
